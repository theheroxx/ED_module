...
Uses the full Math Model with GNN regional adjustments:
    ...
    - GNN regional adjustments (if cluster_id available)
"""
...
def main():
    ...
    st.markdown("### Medically Grounded Baseline + GNN Regional Adjustments")
    st.markdown("---")

    # Show bias map status
    if model.bias_map:
        st.info(f"✅ GNN regional adjustments loaded for {len(model.bias_map)} clusters")
    else:
    ...
    with col1:
        st.markdown("#### Weather")
        temp = st.number_input("Temperature (°C)", -50.0, 60.0, 22.0, step=0.5)
        humid = st.slider("Humidity (%)", 0, 100, 45)
        wind = st.slider("Wind (kph)", 0, 100, 10)
        uv = st.slider("UV Index", 0, 15, 3)

    with col2:
        st.markdown("#### Air Quality")
        st.caption("Enter pollutant concentrations (leave 0 if unknown)")
        
        pm25 = st.number_input("PM2.5 (µg/m³)", 0.0, 500.0, 10.0, step=0.1, key="pm25")
        pm10 = st.number_input("PM10 (µg/m³)", 0.0, 600.0, 0.0, step=0.1, key="pm10")
        o3 = st.number_input("O3 (ppm)", 0.0, 0.5, 0.0, step=0.001, key="o3", format="%.3f")
        no2 = st.number_input("NO2 (ppm)", 0.0, 2.0, 0.0, step=0.001, key="no2", format="%.3f")
        so2 = st.number_input("SO2 (ppm)", 0.0, 1.0, 0.0, step=0.001, key="so2", format="%.3f")
        co = st.number_input("CO (ppm)", 0.0, 50.0, 0.0, step=0.1, key="co")

        # Cluster ID input (optional)
        cluster_id = st.number_input("Cluster ID (optional, -1 for auto)", -1, 5, -1, step=1)
        if cluster_id == -1:
            cluster_id = None
    ...
    if st.button("🚀 Predict Exercise Danger", type="primary", use_container_width=True):
        ...
        # Use the math model (with GNN adjustments)
        ...
        if regional_adjustment != 0:
            adj_text = f"{regional_adjustment:+.2f} points" if regional_adjustment != 0 else "No adjustment"
